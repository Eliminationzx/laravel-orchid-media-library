...
abstract class AbstractMediaListScreen extends AbstractScreen
{
    ...
    protected function registerMediaCollections($model): void
    {
        $this->collections = $model
            ...
            ->pluck('name')
            ->mapWithKeys(
                static fn (string $value): array => [$value => __("media-collections.$value")]
            )
            ->toArray();
    }

    public function getBuilder(Media|Builder|null $builder = null): Builder
    {
        $models = collect(glob(app_path('Models/*.php')))
            ->map(static fn (string $model): string => str($model)
